...
//     body: JSON.stringify({ name, department, email, category, office, message })
//   })
...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (!name || !department || !email || !category || !office || !message) {
      return {
        statusCode: 400,
        headers: { "Content-Type": "application/json" },
    ...
    if ([name, department, email, office].some((f) => f.length > MAX_FIELD)) {
      return {
        statusCode: 400,
        headers: { "Content-Type": "application/json" },
